...
Perhaps even more unusual than the for-loop is the use of a nested while-loop; this is needed in order to navigate between the pages rendered by JavaScript. Basically, the while-loop runs for as long as the prev_page_button (a JavaSCript control) is active, which allows us to access all subpages within each url.  
...
The goal difference between the top and bottom sides is a remarkable 126 goals across just 38 games. This is a staggering difference and the features we build for our model must reflect this. (Introduce goal supremacy here)
